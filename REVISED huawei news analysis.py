...
def plot_top_keywords(country_keywords):
    ...
    # Collect all keywords and their scores across countries
    all_keywords = {country: set([word for word, _ in keywords]) for country, keywords in country_keywords.items()}
    max_score = max(max(score for _, score in keywords) for keywords in country_keywords.values())
    ...
    for i, (country, keywords) in enumerate(country_keywords.items()):
        ...
        axes[i].set_xlim([0, max_score])  # Standardize x-axis across all subplots
...
def plot_top_bigrams(country_bigrams):
    ...
    max_score = max(max(score for _, score in bigrams) for bigrams in country_bigrams.values())
...
# Extract polarity scores for each country
us_polarity = country_polarity[country_polarity['Country'] == 'US']['Polarity'].iloc[0]
uk_polarity = country_polarity[country_polarity['Country'] == 'UK']['Polarity'].iloc[0]
china_polarity = country_polarity[country_polarity['Country'] == 'China']['Polarity'].iloc[0]

# Perform t-tests between each pair of countries
t_stat_us_uk, p_value_us_uk = stats.ttest_ind(us_polarity, uk_polarity)
t_stat_us_china, p_value_us_china = stats.ttest_ind(us_polarity, china_polarity)
t_stat_uk_china, p_value_uk_china = stats.ttest_ind(uk_polarity, china_polarity)

# Perform ANOVA
f_stat, p_value = stats.f_oneway(us_polarity, uk_polarity, china_polarity)

# Prepare data for the table
results = pd.DataFrame({
    'Test - Polarity': ['t-test: US vs. UK', 't-test: US vs. China', 't-test: UK vs. China', 'ANOVA: All countries'],
    'Statistic': [f"{t_stat_us_uk:.2f}", f"{t_stat_us_china:.2f}", f"{t_stat_uk_china:.2f}", f"{f_stat:.2f}"],
    'P-Value': [f"{p_value_us_uk:.2f}", f"{p_value_us_china:.2f}", f"{p_value_uk_china:.2f}", f"{p_value:.2f}"]
})
...
for country, scores in zip(countries, subjectivity_lists):
    stat, p = stats.shapiro(scores)
    print(f"{country}: Stat={stat}, P-value={p}")

# Homogeneity of Variance Test
print("\nLevene’s Test for Equality of Variances:")
stat, p = stats.levene(us_scores, uk_scores, china_scores)
print(f"Stat={stat}, P-value={p}")

# Extract scores for each country
us_scores = country_subjectivity[country_subjectivity['Country'] == 'US']['Subjectivity'].iloc[0]
uk_scores = country_subjectivity[country_subjectivity['Country'] == 'UK']['Subjectivity'].iloc[0]
china_scores = country_subjectivity[country_subjectivity['Country'] == 'China']['Subjectivity'].iloc[0]

# Perform t-tests between each pair of countries
t_stat_us_uk, p_value_us_uk = stats.ttest_ind(us_scores, uk_scores)
t_stat_us_china, p_value_us_china = stats.ttest_ind(us_scores, china_scores)
t_stat_uk_china, p_value_uk_china = stats.ttest_ind(uk_scores, china_scores)

# Perform ANOVA
f_stat, p_value = stats.f_oneway(us_scores, uk_scores, china_scores)

# Prepare data for the table
results = pd.DataFrame({
    'Test - Subjectivity': ['t-test: US vs. UK', 't-test: US vs. China', 't-test: UK vs. China', 'ANOVA: All countries'],
    'Statistic': [f"{t_stat_us_uk:.2f}", f"{t_stat_us_china:.2f}", f"{t_stat_uk_china:.2f}", f"{f_stat:.2f}"],
    'P-Value': [f"{p_value_us_uk:.2f}", f"{p_value_us_china:.2f}", f"{p_value_uk_china:.2f}", f"{p_value:.2f}"]
})
